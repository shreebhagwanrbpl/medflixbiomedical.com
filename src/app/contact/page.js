                ...
                {loading ? (
                  <div className="space-y-4">
                    {[...Array(3)].map((_, i) => (
                      <div
                        key={i}
                        className="h-28 rounded-3xl bg-[#e6f4f2]/60 animate-pulse"
                      />
                    ))}
                  </div>
                ) : contactInfo.length === 0 ? (
                  <div className="rounded-3xl border border-[#cdeae5] bg-white p-8 text-center text-[#496a66]">
                    <Info size={32} className="mx-auto text-[#0b6e69] mb-2" />
                    <p className="font-semibold text-[#133835]">No Contact Information Added</p>
                  ...
                  contactInfo.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-start gap-4 rounded-3xl border border-[#cdeae5] bg-white p-6 shadow-sm transition-all hover:border-[#0b6e69]/40 hover:shadow-md"
                      >
                        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#e6f4f2] text-[#0b6e69] shrink-0">
                          {getFieldIcon(label)}
                        </div>
                        <div className="flex-1">
                          <h4 className="text-xs font-bold uppercase tracking-wider text-[#496a66]">
                            {label}
                          </h4>
                          <div className="mt-2 flex flex-col gap-1.5">
                            {displayValues.map((val, vIdx) => {
                              if (isPhone) {
                                return (
                                  <a
                                    ...
                                    className="text-base sm:text-lg font-bold text-[#133835] hover:text-[#0b6e69] transition-colors inline-block"
                                  >
                                    {val}
                                  </a>
                                );
                              ...
                              if (isEmail) {
                                return (
                                  <a
                                    key={vIdx}
                                    href={`mailto:${val}`}
                                    className="text-base font-bold text-[#133835] hover:text-[#0b6e69] transition-colors inline-block break-all"
                                  >
                                    {val}
                                  </a>
                                );
                              ...
                              return (
                                <p
                                  key={vIdx}
                                  className="text-sm sm:text-base font-bold text-[#133835] leading-relaxed"
                                >
                                  {val}
                                </p>
                              );
          ...
          <div className="mt-12 space-y-4">
            {faqs.map((faq, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-3xl border border-[#cdeae5] bg-[#e6f4f2]/60 overflow-hidden transition-all"
                >
                  <button
                    onClick={() => setOpenFaq(isOpen ? null : idx)}
                    className="w-full flex items-center justify-between p-6 text-left font-bold text-[#133835] hover:text-[#0b6e69]"
                  >
                  ...
                  {isOpen && (
                    <div className="px-6 pb-6 text-sm sm:text-base leading-relaxed text-[#496a66] border-t border-[#cdeae5]/40 pt-4">
                      {faq.a}
                    </div>
                  )}
                </div>
